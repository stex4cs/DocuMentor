...
exports.SwaggerGenerator = exports.HtmlGenerator = void 0;
...
class HtmlGenerator {
    ...
    generateDocs(endpoints, outputPath, apiInfo) {
        logger_1.logger.info(`Generating HTML documentation at ${outputPath}...`);
        try {
            // Read HTML template
            let templatePath = path.join(this.templateDir, 'api.html');
            if (!fs.existsSync(templatePath)) {
                logger_1.logger.warn(`Template not found at ${templatePath}, using default template`);
            ...
            logger_1.logger.info('HTML documentation generated successfully.');
        }
        catch (error) {
            logger_1.logger.error('Error generating HTML documentation:', error);
            throw error;
        }
    }
    /**
     ...
     */
    generateEndpointHtml(endpoint, baseUrl) {
        const title = endpoint.summary || `${endpoint.method} ${endpoint.path}`;
